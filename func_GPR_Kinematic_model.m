function [Next_Steer,Refer_Path,Predict_Path] = func_GPR_Kinematic_model(local_path_x,local_path_y,heading,curvature,total_s,segment_arc_s,VehState)
global VehiclePara;
...
% for i =1:1:N
%     Refer_Path(i,1) = local_path_x(i);
%     Refer_Path(i,2) = local_path_y(i);
%     Refer_Path(i,3) = heading(i);
% end
% 预测时域所需参考路径
Refer_Path(1,:) = [local_path_x(1),local_path_y(1),heading(1)];
size_n = length(total_s);
index = 2;
for i = 2:1:N
    dt_distance = speed * MPCParameters.Ts;
    move_distance = (i-1) * dt_distance;
    while(index < size_n && total_s(index) - total_s(1) < move_distance)
        index = index + 1;
    end
    if(index == size_n )
        Refer_Path(i,1) = Refer_Path(i-1,1) + dt_distance * cos(Refer_Path(i-1,3));
        Refer_Path(i,2) = Refer_Path(i-1,2) + dt_distance * sin(Refer_Path(i-1,3));
        Refer_Path(i,3) = Refer_Path(i-1,3);
    else
       tiny_s =  move_distance - total_s(index-1);
       factor =  tiny_s/segment_arc_s(index);
       Refer_Path(i,1) = local_path_x(index-1) + factor * (local_path_x(index) - local_path_x(index - 1));
       Refer_Path(i,2) = local_path_y(index-1) + factor * (local_path_y(index) - local_path_y(index - 1));
       Refer_Path(i,3) = heading(index-1) + factor * (heading(index) - heading(index-1));
    end
end
%********************************************二.约束及上下界********************************************
...
%********************************************三.目标方程********************************************
objective_func = @(vars)MPC_objective_func(vars,Refer_Path,kappa);
...
function objective_func = MPC_objective_func(vars,Refer_Path,kappa)
global CostWeights;
global MPCParameters;
global Constraints;
